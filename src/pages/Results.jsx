...
export default function Results() {
    ...
    <div className="min-h-screen bg-[#2C1A0E] flex items-center justify-center">
      <div className="text-center px-6">
        ...
        <button onClick={() => navigate('/')}
          className="px-6 py-3 rounded-full font-semibold text-sm text-[#2C1A0E]"
  ...
  return (
    <div className="min-h-[100dvh] bg-[#2C1A0E] text-[#FAF4EC] p-4 sm:p-6 pb-40 overflow-x-hidden relative">
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          SECTION 1 : HEADER (Selfie + Analyse)
          ═══════════════════════════════════════════════════════════════════ */}
      <motion.div
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        className="mb-10 flex flex-row gap-5 items-center bg-white/5 p-5 rounded-[2.5rem] border border-white/10 shadow-2xl relative"
        ...
        <div className="relative shrink-0">
          ...
          <div className="absolute -bottom-2 -right-2 bg-[#C9963A] text-[#2C1A0E] text-[10px] font-black px-2 py-1 rounded-md shadow-lg uppercase">
            Moi
          </div>
        </div>

        {/* Texte (Prénom + Analyse) */}
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          SECTION 2 : ERROR MESSAGE (Si erreur)
          ═══════════════════════════════════════════════════════════════════ */}
      <AnimatePresence>
        {errorMsg && (
          <motion.div
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          SECTION 3 : RESULT IMAGE (Modal qui apparaît après génération)
          ═══════════════════════════════════════════════════════════════════ */}
      <AnimatePresence>
        {resultImage && (
          <motion.div
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          SECTION 4 : STYLES RECOMMENDATIONS (Liste scrollable)
          ═══════════════════════════════════════════════════════════════════ */}
        ...
        {shuffledStyles.map((style, index) => {
          ...
          return (
            <motion.div
              ...
              className="bg-[#3D2616] rounded-[2.5rem] overflow-hidden border border-[#C9963A]/20 shadow-2xl relative">

              {/* Images Grid (3 colonnes) */}
              <div className="grid grid-cols-3 gap-0.5 h-72 bg-black/40 relative">
                
                {/* Colonne 1-2 : Face (2/3) */}
              ...
              <div className="p-6">
                ...
                <button
                  ...
                  disabled={isLoading || !hasCredits() || !hasPaidCredits()}
                  className="w-full py-4 rounded-2xl font-display font-bold text-base shadow-xl active:scale-[0.98] transition-all disabled:opacity-60 text-[#2C1A0E]"
                  style={{ background: 'linear-gradient(135deg, #C9963A, #E8B96A)' }}>
                  {isLoading ? (
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          SECTION 5 : FLOATING CREDITS BUTTON (Fixed Bottom-Right)
          ═══════════════════════════════════════════════════════════════════ */}
      <motion.div
