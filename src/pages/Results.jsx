...
// Pas d'accents directs — Unicode escapes obligatoires
...
// Lire les pages sauvegard\u00e9es en sessionStorage
function getPages() {
  try {
...
// Sauvegarder une nouvelle page (3 styles) en sessionStorage
function pushPage(styles3) {
  try {
    const pages = getPages()
    pages.push({ styles: styles3, ts: Date.now() })
    sessionStorage.setItem(KEY_PAGES, JSON.stringify(pages))
    return pages.length // retourne le nouvel index (base-1)
  } catch { return 1 }
}

// Tirer 3 styles non encore vus pour cette forme de visage
function pick3(faceShape) {
  const seen      = getSeenStyleIds()
  const pages     = getPages()
  const usedInPages = pages.flatMap(p => p.styles.map(s => s.id))
  const available = BRAIDS_DB.filter(s =>
    s.faceShapes.includes(faceShape) &&
    !seen.includes(s.id) &&
    !usedInPages.includes(s.id)
  )
  // Melange
...
export default function Results() {
  ...
  // Pagination
  const [pages, setPages]             = useState([]);       // tableau de pages
  const [pageIdx, setPageIdx]         = useState(0);        // index affich\u00e9 (base-0)
  ...
  // Chargement initial — lire les pages existantes ou cr\u00e9er la premi\u00e8re
  useEffect(() => {
    let existing = getPages()
    if (existing.length === 0) {
      // Premi\u00e8re visite sur Results : cr\u00e9er la page 1
      const styles3 = pick3(faceShape)
      if (styles3.length > 0) {
        pushPage(styles3)
        existing = getPages()
      }
    ...
    setPages(existing)
    setPageIdx(existing.length - 1) // afficher la derni\u00e8re page
  }, []);

  // Styles affich\u00e9s = page courante
  const currentStyles = pages[pageIdx]?.styles || []
  const totalPages    = pages.length

  // Image de r\u00e9f\u00e9rence du style
  function styleImgUrl(style) {
    if (style.localImage) return "/styles/" + style.localImage;
    if (style.image)      return style.image.startsWith("http") ? style.image : style.image;
    return "";
  }
  ...
  const handleTryStyle = async (style) => {
    ...
    try {
      ...
      const refUrl       = window.location.origin + styleImgUrl(style);

      if (!selfieBase64) throw new Error("Selfie introuvable. Prends une photo d\u2019abord.");
      if (!refUrl || refUrl.includes("undefined")) throw new Error("Image de coiffure introuvable.");
  ...
  const handleShare = async (styleId) => {
    try {
      if (navigator.share) {
  ...
  if (!currentStyles.length) return (
    <div className="min-h-screen bg-[#2C1A0E] flex items-center justify-center">
      <div className="text-center px-6">
        ...
        <p className="text-white text-xl font-semibold mb-2">Quelle tresse aujourd\u2019hui ?</p>
        <p className="text-gray-400 text-sm mb-6">Prends un selfie pour d\u00e9couvrir les styles qui sublimeront ton visage.</p>
  ...
  return (
    <div className="min-h-[100dvh] bg-[#2C1A0E] text-[#FAF4EC] p-4 sm:p-6 pb-40 overflow-x-hidden relative">
      ...
      {/* HEADER */}
      <div className="mb-8 flex flex-row gap-5 items-center bg-white/5 p-5 rounded-[2.5rem] border border-white/10 shadow-2xl">
      ...
      {/* BARRE PAGINATION — juste sous le header */}
      {totalPages > 1 && (
          ...
          <motion.div ref={resultRef}
            ...
            <div className="p-5">
              ...
              <p className="text-[11px] mt-1 opacity-70">
                {isFallback ? "Aper\u00e7u bas\u00e9 sur ta forme de visage" : "Ce style te met vraiment en valeur. Montre-le \u00e0 ta coiffeuse !"}
              </p>
            </div>
      ...
      <div className="space-y-12">
        {currentStyles.map((style) => {
          const imgSrc = style.views?.face || "/styles/" + (style.localImage || style.image || "");
          const imgBack = style.views?.back || imgSrc;
          const imgTop  = style.views?.top  || imgSrc;
          return (
            <div key={style.id} className="bg-[#3D2616] rounded-[2.5rem] overflow-hidden border border-[#C9963A]/20 shadow-2xl">
              <div className="grid grid-cols-3 gap-0.5 h-72 bg-black/40">
                <div className="col-span-2 h-full overflow-hidden">
                  <img src={imgSrc} className="w-full h-full object-cover object-top cursor-pointer"
                    onClick={() => setZoomImage(imgSrc)} alt={style.name} />
                </div>
