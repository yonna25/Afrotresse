...
export default function Results() {
  ...
  const handleTryStyle = async (style) => {
    ...
    try {
      ...
      if (upError) throw new Error("Erreur d'envoi de la photo vers Supabase.");
      ...
      if (!res.ok) throw new Error(data.error || "L'IA a échoué.");
  ...
  return (
    <div className="min-h-screen bg-[#2C1A0E] text-[#FAF4EC] p-4 pb-32">
      ...
      <AnimatePresence>
        {resultImage && (
          <motion.div ref={resultRef} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10 bg-[#3D2616] rounded-[2.5rem] overflow-hidden border-2 border-[#C9963A] shadow-2xl">
            <img src={resultImage} className="w-full" alt="Résultat IA" />
            <div className="p-4">
              <button onClick={() => setResultImage(null)} className="w-full py-4 bg-white/10 rounded-2xl text-sm font-bold">Essayer un autre modèle</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="space-y-10">
        {currentResults.map((style) => (
          <div key={style.id} className="bg-[#3D2616] rounded-[2.5rem] overflow-hidden border border-[#C9963A]/20 shadow-xl">
            <div className="h-72 overflow-hidden bg-black/20">
              <img 
                src={getImgPath(style.localImage || style.image)} 
                className="w-full h-full object-cover object-top" 
                alt={style.name} 
                onError={(e) => { e.target.src = "https://placehold.co/400x600?text=Image+Introuvable"; }}
              />
            </div>
            <div className="p-6">
      ...
      <div onClick={() => navigate("/credits")} className="fixed bottom-10 right-5 bg-[#C9963A] text-[#2C1A0E] w-14 h-14 rounded-2xl flex flex-col items-center justify-center shadow-2xl border-2 border-white/20 active:scale-90 transition-transform">
