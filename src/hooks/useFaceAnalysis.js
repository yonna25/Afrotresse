...
// ── Hash SHA-256 d'un Blob → hex string (cache image)
...
export async function analyzeFaceWithAI(photoData, timeoutMs = 10000) {
  ...
  try {
    ...
    // 🔒 CACHE IMAGE — hash SHA-256 avant envoi
    // Si même photo déjà analysée dans cette session → retour immédiat sans crédit
    let imageHash = null;
    try {
    ...
    // 🔒 REQUEST ID — UUID unique par envoi (anti double-consommation serveur)
    ...
    try {
      response = await fetch("/api/analyze", {
        ...
        body: JSON.stringify({ faceShape, requestId }),
    ...
    // 🔒 CACHE IMAGE — stocker le résultat pour cette session
    if (imageHash) {
      try {
