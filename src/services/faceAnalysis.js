...
export async function analyzeFace(photoBlob) {
  ...
  try {
    result = await analyzeFaceWithAI(photoBlob);
  } catch (err) {
    // Si l'erreur vient des crédits ou du rate limit → on la propage
    ...
    if (
      ...
      msg.includes("429") ||
      msg.includes("409") ||
      msg.includes("déjà traitée") ||
      msg.includes("déjà effectuée")
    ) {
      throw err;
    }
    // Si MediaPipe échoue → fallback oval, on continue
    console.warn("MediaPipe échoué, fallback oval:", msg);
    result = { faceShape: "oval", confidence: 75 };
  }
